...
## 삭제 파일 복구 실습
파일 시스템의 연결 고리가 끊어졌을 때, 디지털 포렌식 도구를 활용하여 삭제된 파일의 흔적을 탐색하고 복구(Export)하는 과정을 실습합니다.
...
#### 1. `root` 폴더 및 하위 파일 시스템 내부를 탐색하여 X 표시가 되어 있는 삭제된 파일 항목을 확인합니다.
...
#### 2. 복구하고자 하는 파일(예: `Grogu.jpg`)을 우클릭한 뒤 `Export Files...`를 선택하여 원하는 경로로 추출합니다.
...
1. 생성한 케이스(Case) 내의 `Data Source Tree` 하위의 `Deleted Files` 항목을 탐색합니다.
2. FTK Imager와 동일하게 복구 대상 파일을 우클릭한 뒤, `Extract File(s)`을 수행하여 파일을 복구합니다.
...
#### [원인 분석]
* **파일 시스템 및 물리적 특성:** 실습에 사용된 USB 드라이브의 경우, 파일 삭제 시 휴지통을 거치지 않고 파일 시스템(FAT32/NTFS) 상에서 메타데이터의 연결 고리가 즉시 끊어지는 영구 삭제 구조를 가집니다.
* **데이터 영역 파괴:** 파일의 메타데이터(이름, 경로 등) 정보는 남아있어 도구상에 노출되었으나, 실제 파일 데이터가 저장되어 있던 클러스터(Cluster) 영역의 데이터가 이미 유실되었거나 다른 데이터로 덮어씌워진(**Overwrite**) 상태입니다. 즉, 껍데기만 복구되고 알맹이(데이터 영역)가 파괴된 전형적인 파일 손상 현상입니다.

#### [향후 계획 및 대안 제시]
1. **파일 카빙(File Carving) 도입:** 파일 시스템의 메타데이터가 제 기능을 하지 못할 때는, 파일 시스템 구조를 거치지 않고 디스크의 비할당 영역(Unallocated Space)을 직접 스캔하여 파일 고유의 특징적인 바이너리 패턴(헤더 및 푸터 시그니처)을 기준으로 파일을 복원하는 '파일 카빙' 기법이 필요합니다. 
2. **HxD 헥스 에디터(Hex Editor) 활용:** 과거 학습 경험을 바탕으로, `HxD` 도구를 이용해 손상된 파일의 헥사(Hex) 값을 직접 분석하고 헤더/푸터 시그니처를 수동으로 조작하여 구조를 정상화하는 기법을 적용해 볼 수 있습니다.

> **결론:** 현재 일반적인 내보내기 기능으로는 파일 정상화가 불가능하므로, 추후 별도의 실습을 통해 **파일 카빙 및 HxD를 활용한 시그니처 기반 수동 복구**를 적용하여 원본 이미지 복원을 시도하고 그 결과를 추가로 기록할 예정입니다.
